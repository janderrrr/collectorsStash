...
export default class VendorEventClient extends BindingClass {
    constructor(props = {}) {
        ...
        const methodsToBind = ['clientLoaded','getVendor', 'getIdentity', 'logout','login',
        'getAllVendors', 'getAllEvents', 'getOneEvent', 'createVendor', 'viewVendorsForEvent'];
...
//
    ...
    async viewVendorsForEvent(id, date, errorCallback) {
        try {
            const response = await this.axiosClient.get(`event/${id}/${date}/vendors`)
            return response.data.vendorsList;
        } catch (error) {
            this.handleError(error, errorCallback)
        }
    }
